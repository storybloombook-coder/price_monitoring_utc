...
(() => {
  ...
  const RU = {
    ...
    'Retry only marketplaces without a collected price': 'Перепроверить только маркетплейсы без собранной цены',
    'All enabled marketplaces already have a collected price for this model.': 'Для этой модели уже собраны цены со всех включённых маркетплейсов.',
    'Safe 5 + 5 + 10 batches, browser hand-off and cooldown continuation are automatic.': 'Пакеты 5 + 5 + 10, передача работы другому браузеру и продолжение после паузы выполняются автоматически.',
    'Retry pass': 'Круг перепроверки',
    'will not roll over to previously visited models.': 'не перейдёт повторно к уже пройденным моделям.',
  ...
  const RU_RULES = [
    ...
    [/^(\d+) unresolved checks queued\. Retry pass (\d+) will not roll over to previously visited models\.$/, m => `В очередь поставлено проверок: ${m[1]}. Круг перепроверки ${m[2]} не перейдёт повторно к уже пройденным моделям.`],
    [/^Retry pass (\d+) will not roll over to previously visited models\.$/, m => `Круг перепроверки ${m[1]} не перейдёт повторно к уже пройденным моделям.`],
    ...
    [/^Refreshing (\d+) marketplaces without a collected price\.$/, m => `Перепроверяются маркетплейсы без собранной цены: ${m[1]}.`],
    [/^Switched Salidzini collection to (.+)$/, m => `Сбор Salidzini переключён на ${m[1]}`],
    [/^(.+) activated automatically$/, m => `${m[1]} активирован автоматически`],
    [/^Automatic Salidzini cooldown · resuming in (\d+) s$/, m => `Автоматическая пауза Salidzini · продолжение через ${m[1]} с`],
  ...
  const PHRASES = [
    ...
    ['pages in active browser', 'страниц в активном браузере'],
