...
(() => {
  ...
  const RU = {
    ...
    'Auto uses the connected extension and one persistent working tab. Salidzini requests are spaced by at least 12 seconds and a page may stabilize for up to 30 seconds. CAPTCHA pauses the whole automatic batch immediately; two consecutive unavailable pages do the same before more requests are sent. Complete the check in the retained tab, then retry a batch of 5. Manual mode sends no automatic requests. No retailer pages are opened.': 'Автоматический режим использует подключённое расширение и одну постоянную рабочую вкладку. Запросы Salidzini выполняются с интервалом не менее 12 секунд, а стабилизация страницы ожидается до 30 секунд. CAPTCHA немедленно приостанавливает весь автоматический пакет; то же происходит после двух недоступных страниц подряд, до отправки следующих запросов. Завершите проверку в сохранённой вкладке, затем повторите пакет из 5 позиций. В ручном режиме автоматических запросов нет. Сайты магазинов напрямую не открываются.',
