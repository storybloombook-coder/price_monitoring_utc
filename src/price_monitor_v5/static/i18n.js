...
(() => {
  ...
  const RU = {
    ...
    'Checking Edge extension…': 'Проверка расширения Edge…',
    ...
    'Edge extension status unavailable': 'Статус расширения Edge недоступен',
    ...
    'Search TCL + model and read seller offers on the matching comparison page. If no match is found, retry with one, then two trailing characters removed. S45HE / S45H and S55HE / S55H are explicit regional aliases; other variants require review, never automatic price acceptance. Delivery time alone does not confirm stock. No retailer pages are opened.': 'Поиск выполняется по TCL + модели, затем на странице сравнения считываются предложения продавцов. Если точная модель не найдена, запрос повторяется с удалением одного, затем двух последних символов. S45HE / S45H и S55HE / S55H — подтверждённые региональные варианты; остальные варианты требуют проверки и их цены не принимаются автоматически. Срок доставки сам по себе не подтверждает наличие. Сайты магазинов напрямую не открываются.',
    'Auto uses the connected v5.0.8+ extension and one working tab, reads all result pages with pauses and finishes only after exact listings match the reported count. Geedo recommendations are excluded. CAPTCHA pauses the browser queue for manual verification. Manual mode sends no automatic requests: use Send to PriceMonitor, review and mark all pages reviewed to finish. No retailer pages are opened.': 'Автоматический режим использует подключённое расширение v5.0.8+ и одну рабочую вкладку, последовательно читает страницы выдачи с паузами и завершает проверку после обработки точных объявлений. Рекомендации Geedo исключаются. CAPTCHA переводит только текущую проверку в ручной режим. В ручном режиме автоматических запросов нет: используйте «Send to PriceMonitor», проверьте предложения и подтвердите, что все страницы просмотрены. Сайты магазинов напрямую не открываются.',
    ' On every marketplace, if the original SKU is not found, search again with one, then two trailing characters removed (minimum 3 characters, letters and digits). Known S45HE/S45H and S55HE/S55H aliases are accepted; other candidates need review. CAPTCHA, network errors and unknown markup stop automatic fallback.': ' На каждом маркетплейсе, если исходный SKU не найден, поиск повторяется с удалением одного, затем двух последних символов (минимум 3 буквенно-цифровых символа). Известные пары S45HE/S45H и S55HE/S55H принимаются как варианты; остальные кандидаты требуют проверки. CAPTCHA, сетевые ошибки и неизвестная разметка останавливают автоматический повтор.',
    ...
    'Capture again': 'Собрать повторно', 'Possible variants — review before entering a price': 'Возможные варианты — проверьте перед вводом цены',
    ...
    'The selected check is capturing again.': 'Выбранная проверка собирается повторно.',
    ...
    'Marketplace-only · 1 request per marketplace at a time · 3 s pacing · 4 h complete-price cache · manual entry available during protection cooldown': 'Только маркетплейсы · по одному запросу на площадку · интервал 3 с · кеш полных цен 4 ч · во время защитной паузы доступен ручной ввод',
  ...
  const PHRASES = [
    ...
    [' · recovery polling', ' · резервный опрос'], ['Edge extension connected', 'Расширение Edge подключено'],
